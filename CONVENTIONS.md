...
- Run `python scripts/run_all_smokes.py` before claiming anything works. Expect 67 suites, 1752 checks, ALL GREEN **on a checkout**. Off a zipball extract - which is what the data machine runs - it is 1743: nine checks in `smoke_buildid` need git to test the archive path and report SKIPPED without it. Both numbers were measured. Do not quote the checkout number to the data machine; that is how a correct run gets read as a failure.
