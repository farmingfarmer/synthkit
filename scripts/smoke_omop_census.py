...
# The real headers live in one place only - the fixture generator - so
# a header can never drift between what is generated and what the
# census is asserted to read back.
from make_omop_fixture import OMOP_HEADERS as HEADERS   # noqa: E402
...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        check("concept is projected at its whole size, never sampled, "
              "and points at the filter that shrinks it",
              any(p["table"] == "concept"
                  and p["mode"] == "whole (sampler filters)"
